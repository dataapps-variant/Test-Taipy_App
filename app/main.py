...
# Tables - initialize with column headers
active_regular_df = pd.DataFrame(columns=["App", "Plan", "Metric"])
active_crystal_df = pd.DataFrame(columns=["App", "Plan", "Metric"])
inactive_regular_df = pd.DataFrame(columns=["App", "Plan", "Metric"])
inactive_crystal_df = pd.DataFrame(columns=["App", "Plan", "Metric"])
...
def process_pivot_data(pivot_data, selected_metrics, is_crystal_ball=False):
    """Process pivot data into actual PIVOT TABLE structure with dates as columns
    
    Uses simple column names without special characters for Taipy compatibility.
    """
    ...
    # Format: "Jan10", "Jan09", etc.
    date_col_names = []
    date_map = {}  # maps original date -> column name
    for d in unique_dates:
        if hasattr(d, 'strftime'):
            col_name = d.strftime("%b%d")  # "Jan10" format - no special chars
        else:
            col_name = str(d).replace("/", "").replace("-", "")[:6]
        date_map[d] = col_name
    ...
    # Build rows with ALL columns explicitly
    rows = []
    for app_name, plan_name in plan_combos:
        for metric in selected_metrics:
            ...
            for d in unique_dates:
                ...
                val = format_metric_value(raw_value, metric, is_crystal_ball)
                row[col_name] = val if val is not None else ""
            rows.append(row)
    
    if not rows:
        return pd.DataFrame(columns=["App", "Plan", "Metric"] + date_col_names)
    
    df = pd.DataFrame(rows)
    
    # Ensure column order
    all_cols = ["App", "Plan", "Metric"] + date_col_names
...
<|{active_selected_apps}|selector|lov={active_app_options}|multiple|on_change=on_active_app_change|>

**Select Plans** (tick to select)

<|{active_selected_plans}|selector|lov={active_plan_options}|multiple|>
...
<|{active_regular_df}|table|page_size=15|show_all=True|width=100%|>

### 🔮 Crystal Ball Data

<|{active_crystal_df}|table|page_size=15|show_all=True|width=100%|>
...
<|{inactive_selected_apps}|selector|lov={inactive_app_options}|multiple|on_change=on_inactive_app_change|>

**Select Plans** (tick to select)

<|{inactive_selected_plans}|selector|lov={inactive_plan_options}|multiple|>
...
<|{inactive_regular_df}|table|page_size=15|show_all=True|width=100%|>

### 🔮 Crystal Ball Data

<|{inactive_crystal_df}|table|page_size=15|show_all=True|width=100%|>
...
def load_active_data(state: State):
    ...
    try:
        ...
        # Debug logging
        logger.info(f"Regular table: {len(state.active_regular_df)} rows, columns: {list(state.active_regular_df.columns)}")
        logger.info(f"Crystal table: {len(state.active_crystal_df)} rows, columns: {list(state.active_crystal_df.columns)}")
        if len(state.active_regular_df) > 0:
            logger.info(f"Regular sample row: {state.active_regular_df.iloc[0].to_dict()}")

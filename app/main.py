...
# Get the CSS file path - works both locally and in Docker
_script_dir = os.path.dirname(os.path.abspath(__file__))
_css_path = os.path.join(os.path.dirname(_script_dir), "styles", "main.css")
if not os.path.exists(_css_path):
    _css_path = "styles/main.css"  # Fallback for Docker working directory

# Create GUI instance
gui = Gui(pages=pages, css_file=_css_path)

if __name__ == "__main__":
    # Preload data at startup (optional - gracefully handle failures)
    ...
    try:
        ...
        logger.warning(f"⚠️ Preloading skipped (will load on first request): {e}")
    
    # Run the GUI
    port = int(os.environ.get("PORT", 8080))
    logger.info(f"🚀 Starting Taipy GUI on port {port}...")
    gui.run(
        title=APP_TITLE,
        host="0.0.0.0",
        port=port,
